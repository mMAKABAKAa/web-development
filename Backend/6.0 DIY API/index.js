...
app.get("/filter",(req,res)=>{
  ...
  const filteredJokes = jokes.filter(joke => joke.jokeType.toLowerCase() === type);
...
app.put("/jokes/:id", (req,res)=>{
  const id =parseInt(req.params.id);
  const newJoke={id:id,jokeText:req.body.text,jokeType:req.body.type};
  const foundIndex=jokes.findIndex((joke)=>joke.id===id);
  jokes[foundIndex]=newJoke;
  res.json(newJoke);
});
//6. PATCH a joke
...
app.delete("/jokes/:id", (req,res)=>{
  ...
  if (foundIndex>-1) {
    ...
    res.status(404).json({error:`Joke with id: ${id} not found. No joke was deleted.`})
  }
});
//8. DELETE All jokes
